...
- [ ] **Implement Watermarking Service:** Use `pdf-lib` (for PDFs) or `sharp` (for images). Create a service function `watermarkFile(filePath, text)` that returns a buffer/stream.
- [ ] **Implement "Secure View" Endpoint:** A GET route `/:linkId` that validates and streams the watermarked file.
